...
(defn- enter-vertex [{current-vertex :current-vertex
...
(defn- leave-vertex [{current-vertex :current-vertex
                      previous-vertex :previous-vertices}]
  (when previous-vertex
    (swap! visited-set update-in
...
(defn- allow-traversal [{next-vertex :next-vertex}]
...
(defn run [ggraph]
  (let [visited-set (atom {})
        ...
        dsf-callbacks {:enter-vertex enter-vertex
